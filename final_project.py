...
class Dataset:
    def __init__(self):
        self.sample_names = np.array([])
        self.sample_status = np.array([])
        self.sample_tissue = np.array([])
        self.gene_names = np.array([])
        self.data = np.array([])
...
def warn_data(ds):
    if np.any(np.isnan(ds.data)):
        print("WARNING: Data contains NAN")
    if np.any(np.sum(ds.data, axis=0) == 0):
        print("WARNING: Data contains zero-sum rows")


def nomalize_data(ds, times_sigma):
    valid = np.ones(len(ds.data)).astype(np.bool)
...
def sum_identical_gene_names(ds):
    for i in range(len(ds.gene_names)):
        ds.gene_names[i] = ds.gene_names[i].split(".")[0]

    s = np.argsort(ds.gene_names)
    ds.gene_names = ds.gene_names[s]
    ds.data = ds.data[:, s]

    if len(np.unique(ds.gene_names)) == len(ds.gene_names):
        return

    m = 0
    i = 0

    while i < len(ds.gene_names):
        j = i+1
        while j < len(ds.gene_names) and ds.gene_names[i] == ds.gene_names[j]:
            j += 1

        ds.gene_names[m] = ds.gene_names[i]
        if i+1 < j:
            ds.data[:, m] = np.sum(ds.data[:, i:j], axis=1)
        else:
            ds.data[:, m] = ds.data[:, i]
    ...
    ds.gene_names = ds.gene_names[0:m]
    ds.data = ds.data[:, 0:m]

my_ds = Dataset()
my_ds.sample_names = sample_names
my_ds.sample_tissue = sample_tissue
my_ds.sample_status = sample_status
my_ds.gene_names = gene_names
my_ds.data = data

my_ds.save('pickled_dataset.pkl')


np.save('sample_names_pickled', sample_names)
# x = np.load('outfile_name.npy') # loads your 'saved array file' into variable x

np.save('sample_status_pickled', sample_status)

np.save('sample_tissue_pickled', sample_tissue)

np.save('gene_names_pickled', gene_names)

np.save('data', data)

full_df = pd.read_pickle('final_df_pickled.pkl')

full_df.head()

brca_df = full_df.loc[full_df['tissue'] == 'brca']


breast_df = full_df.loc[full_df['tissue'] == 'breast']


brca_df.head()

concat_breast_df = pd.concat([brca_df, breast_df], sort=False)

concat_breast_df.head()

df.to_pickle('./concat_breast_df.pkl')
